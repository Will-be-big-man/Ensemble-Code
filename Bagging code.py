...
# 그래서 총 train_data, test_data, train_targets_data, test_targets_data 4개만 남음
...
for j in range(no_cases):
 ...
 for m in range(len(tmp)):
  ...
  for i in range(k):
   ...
   imagename2= imagename + "bagging_" + "k" + str(i) + ".png"
   plt.savefig(imagename2)
   plt.close()
    

   if(i ==0 ):
    prediction_test_0 = model.predict(test_data)
   if(i ==1):
    prediction_test_1 = model.predict(test_data)
   if(i ==2):
    prediction_test_2 = model.predict(test_data)
   if(i ==3):
    prediction_test_3 = model.predict(test_data)
   if(i ==4):
    prediction_test_4 = model.predict(test_data)
   y = model.predict(test_data)
		
   filename2 = filename + "bagging" + "k" + str(i) + ".txt"
...
final_bagging = np.mean((prediction_test_0, prediction_test_1, prediction_test_2, prediction_test_3, prediction_test_4),axis =0)

filename2 = filename + "output_bagging" + ".txt"
with open(filename2,'w') as p:
 sum1 = 0
 sum2 = 0
 p.write(header1)
 ...
 p.write(header2)
 p.write("\n")
 for n in range(len(final_bagging)):
  tmpPredEGR = final_bagging[n]
  a = np.array2string(tmpPredEGR)
  a = a.strip('[')
  a = a.strip(']')
  p.write(a)
  ...
  sum1 = sum1 + (float(a)-float(b))**2
  sum2 = sum2 + abs(float(a)-float(b))
 mse = sum1/len(test_targets_data)
 mae = sum2/len(test_targets_data)
...
label1 = "Acc"
plt.plot(test_targets_data, final_bagging, 'b', label=label1)
plt.legend()
plt.xlabel('prediction')
plt.ylabel('y_test')

imagename3= "last_output"+ ".png"
plt.savefig("last_ouput_bagging.png")
plt.close()
